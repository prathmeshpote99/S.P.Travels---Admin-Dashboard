...
import { getCustomerById, updateCustomer } from "../../services/Apis";
...
function EditForm() {
  ...
  const [alert, setAlert] = useState({ visible: true, message: "", color: "" });
  ...
  useEffect(() => {
    const fetchUserByID = async (userId) => {
      try {
        ...
        setPrnNo(userData.prnNo);
        setDocNo(userData.docNo);
        setCorporate(userData.corporate);
        setAgentName(userData.agentName);
        setBookedBy(userData.bookedBy);
        setModifiedBy(userData.modifiedBy);
        setBookName(userData.bookName);
        setReceiptNo(userData.receiptNo);
        setBookingDate(userData.bookingDate.slice(0, 10));
        setBookingTime(userData.bookingTime.slice(0, 5));
        setFullName(userData.fullName);
        setEmail(userData.email);
        setPhoneNo(userData.phoneNo);
        setAlternatePhoneNo(userData.alternatePhoneNo);
        setAddress(userData.address);
        setPickupDate(userData.pickupDate.slice(0, 10));
        setPickupTime(userData.pickupTime.slice(0, 5));
        setPickupPoint(userData.pickupPoint);
        setPickupAddress(userData.pickupAddress);
        setPassengerRemark(userData.passengerRemark);
        setArea(userData.area);
        setTrip(userData.trip);
        setAirportType(userData.airportType);
        setFlightNo(userData.flightNo);
        setFlightDate(userData.flightDate.slice(0, 10));
        setFlightETA(userData.flightETA.slice(0, 5));
        setDepartDate(userData.departDate.slice(0, 10));
        setDepartTime(userData.departTime.slice(0, 5));
        setCollectionThrough(userData.collectionThrough);
        setTotalSeats(userData.totalSeats);
        setTicketAmount(userData.ticketAmount);
        setDiscountAmount(userData.discountAmount);
        setCgstAmt(userData.cgstAmt);
        setSgstAmt(userData.sgstAmt);
        setPickupCharges(userData.pickupCharges);
  ...
  const onSubmit = async () => {
    ...
    const response = await updateCustomer(userId, newData);
  ...
  return (
    <>
      <div className="alertbox_editform d-flex justify-content-end align-items-center fs-6">
                ...
                <Card id="delete-account">
                  ...
                  <Modal centered isOpen={modalOpen} toggle={() => setModalOpen(false)}>
                    <ModalHeader toggle={() => setModalOpen(false)}>
                      Hurray..! Ticket Updated Successfully
                    </ModalHeader>
                    <ModalBody>
